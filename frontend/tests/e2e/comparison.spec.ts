...
test.describe("Product comparison flow", () => {
  ...
  test("shows empty state with example prompts on load", async ({ page }) => {
    ...
    await expect(page.getByText(/gaming laptop/i)).toBeVisible();
  });

  test("chat input is visible and accepts text", async ({ page }) => {
    const input = page.getByRole("textbox");
    await input.fill("Find me a OnePlus 12");
    await expect(input).toHaveValue("Find me a OnePlus 12");
  });

  test("clicking example prompt fills input", async ({ page }) => {
    const prompt = page.getByText(/gaming laptop/i).first();
    await prompt.click();
